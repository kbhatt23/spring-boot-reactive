 ...
 * 
 * //@Configuration //@EnableWebFluxSecurity public class ReactiveSpringSecurity
 * {
 * 
 * @Bean public MapReactiveUserDetailsService authentication() { UserDetails
 * userDetails1 = User.withDefaultPasswordEncoder() .username("kbhatt23")
 * .password("jaishreeram") .roles("USER") .build();
 * 
 * UserDetails userDetailsAdmin = User.withDefaultPasswordEncoder()
 * .username("sitaRam") .password("ramadutahanuman") .roles("ADMIN") .build();
 * 
 * return new MapReactiveUserDetailsService(userDetails1,userDetailsAdmin); }
 * 
 * @Bean public SecurityWebFilterChain authorize(ServerHttpSecurity http) {
 * return http.httpBasic() .and() .authorizeExchange() .anyExchange()
